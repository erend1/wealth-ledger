...
| M002 | Accepted | Retry-safe transaction submission and resolvable transaction readback | ADR-006; implementation and verification pending |
...
## Immediate accepted milestone

The only accepted implementation milestone is
[`M002_transaction_submission_and_readback.md`](milestones/M002_transaction_submission_and_readback.md).

M002 deliberately hardens the existing write boundary before another command
endpoint is added. It separates a client's retry identity from a provider's
external transaction reference, makes created transaction URLs resolvable, and
adds tests for equivalent replay, conflicting replay, and concurrent
submission.

M002 and its dedicated retry-identity decisions were accepted by the human
owners on 2026-08-24. It may now move to In Progress when implementation begins.
...
- duplicate submissions cannot create duplicate transactions;
- every posted transaction can be read back and inspected;
